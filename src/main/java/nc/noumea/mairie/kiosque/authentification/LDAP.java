...
public class LDAP {
	...
	public synchronized static Hashtable<String, String> getHashParametres() {
		if (HASHPARAMETRE == null) {
			...
			for (String cle : Executions.getCurrent().getDesktop().getWebApp()
					.getInitParameterNames()) {
				HASHPARAMETRE.put(cle, Executions.getCurrent().getDesktop()
						.getWebApp().getInitParameter(cle));
	...
	// * true ou false
	// */
	// public static ArrayList<Object> chercherUserLDAPAttributs(String
	// userName) {
	 ...
	 *         correspondant
	 */
	public static ArrayList<Hashtable<String, Object>> chercherUserLDAPAttributs(
			List<String> cles, String critere, String userName) {
		return chercherUserLDAPAttributs(getHashParametres(), cles, critere,
				userName);
	}
	 ...
	 */
	public static ArrayList<Hashtable<String, Object>> chercherUserLDAPAttributs(
			Hashtable<String, String> parametres, List<String> cles,
			String critere, String userName) {
		...
		try {
			...
			try {
				...
				System.out
						.println("MairieLDAP : Impossible de récupérer le contexte admin");
				throw admin;
			}
			...
			// "(cn="+user+")", constraints);
			// String critere =
			// (String)parametres.get("CRITERE_RECHERCHE_LDAP");

			NamingEnumeration<SearchResult> enume = contextAdmin.search(
					(String) parametres.get("BASE_LDAP"), "(" + critere + "="
	...
	// chercherUserLDAPAttributs(Hashtable<String, String> parametres, String
	// userName) {
	//
	...
	// constraints.setSearchScope(SearchControls.SUBTREE_SCOPE);
	//
	// //hurle NamingEnumeration enum = ctx.search(BASE_LDAP, "(cn="+user+")",
	// constraints);
	// String critere = (String)parametres.get("CRITERE_RECHERCHE_LDAP");
	//
	// NamingEnumeration<SearchResult> enume =
	// contextAdmin.search((String)parametres.get("BASE_LDAP"),
	 ...
	 * @return boolean
	 */
	public static boolean controlerHabilitation(String userName,
			String userPassword) {
		return controlerHabilitation(getHashParametres(), userName,
				userPassword);
	}
	 ...
	 * @return boolean
	 */
	public static boolean controlerHabilitation(
			Hashtable<String, String> parametres, String userName,
			String userPassword) {
		...
		try {
			...
			newEnv.put(Context.INITIAL_CONTEXT_FACTORY,
					(String) parametres.get("INITCTX_LDAP"));
			...
			newEnv.put(Context.SECURITY_PRINCIPAL, dn + ","
					+ (String) parametres.get("BASE_LDAP"));
			...
			try {
				...
				System.out
						.println("MairieLDAP : Mauvais mot de passe pour le user "
								+ userName + " --> " + auth);
				return false;
			} catch (NameNotFoundException auth2) { // Nom inexistant
				System.out.println("MairieLDAP : User " + userName
						+ " inexistant dans ldap --> " + auth2);
				return false;
			}
	 ...
	 */
	private static ArrayList<String> construitListeHostLDAP(
			Hashtable<String, String> parametres) {
	 ...
	 * Methode controlerHabilitation qui retourne true ou false
	 */
	private static DirContext getAdminContext(
			Hashtable<String, String> parametres) throws Exception {
		...
		for (Iterator<String> iter = listeHostLDap.iterator(); iter.hasNext();) {
			...
			try {
				...
				System.out
						.println("MairieLDAP : Impossible de récupérer le contexte admin avec "
								+ HOST_LDAP);
				HOST_LDAP = null;
			}
		...
		if (HOST_LDAP == null) {
			...
			throw new Exception(
					"MairieLDAP : Paramètre HOST_LDAP non trouvé ou incorrect.");
		}
