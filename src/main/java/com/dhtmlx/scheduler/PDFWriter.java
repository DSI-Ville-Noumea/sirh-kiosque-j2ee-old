...
public class PDFWriter {
	...
	private double timelineLeft = 120;
	...
	private double timeLineEventHeight = 20;
	...
	private String bgColorLineTimelinePair = "F0D0A0";
	private String bgColorLineTimelineImpair = "F8EBD8";
	private String bgColorLineTimelineService = "EE760B";
	
	private String bgColorDate = "AC5B29";
	...
	private String textColorTodayLabel = "4880BA";
	...
	private void weekHeaderDraw() throws Exception {
		double[] bgColor = RGBColor.getColor(this.bgColorDate);
		double[] borderColor = RGBColor.getColor(this.headerLineColor);
		...
		for (int i = 0; i < cols.length; i++) {
			...
			double text_x = (width - this.f1.stringWidth(text.getText())) / 2;
			double text_y = (height + this.f1.getSize()) / 2;
			text.setColor(RGBColor.getColor(headerLineColor));
			text.setPosition(text_x, text_y);
			text.placeIn(cell);
			text.drawOn(this.page);
	...
	private void timelineContainerDraw() throws Exception {
		double[] bgColorPair = RGBColor.getColor(this.bgColorLineTimelinePair);
		double[] bgColorImpair = RGBColor.getColor(this.bgColorLineTimelineImpair);
		double[] bgColorService = RGBColor.getColor(this.bgColorLineTimelineService);
		double[] borderColor = RGBColor.getColor(this.headerLineColor);
		double[] headerLineColor = RGBColor.getColor(this.headerLineColor);
		double[] scaleOneColor = RGBColor.getColor(this.scaleOneColor);
		String[] rows = this.parser.weekRowsParsing();
		String[][] rowsWithColorText = this.parser.weekRowsParsingWithColorText();
		double width = this.leftScaleWidth;
		double height = timeLineEventHeight;
		...
		for (int i = 0; i < rows.length; i++) {
			...
			String colorText = rowsWithColorText[i][1];
			
			double[] colorBgDouble = null;
			double[] textColor = null;
			double text_x = 10;
			if(colorText.equals("rgb(255, 255, 255)")) {
				colorBgDouble = bgColorService;
				textColor = RGBColor.getColor(this.headerLineColor);
				width = 200;
				text_x = 5;
			}else{
				colorBgDouble = isPair ? bgColorPair : bgColorImpair;
				textColor = RGBColor.getColor("000000");
				width = this.leftScaleWidth;
			}
			
			//////// cette partie cree les entetes "nom agent" a gauche du tableau
			Box cell = new Box();
			cell.setSize(width, height);
			cell.setColor(colorBgDouble);
			cell.setFillShape(true);
			cell.setPosition(x, y);
			...
			/////// cette partie cree le fond de couleur de la partie centrale du tableau ///
			Box cellRight = new Box();
			cellRight.setSize(this.pageWidth-width, height);
			cellRight.setColor(colorBgDouble);
			cellRight.setFillShape(true);
			cellRight.setPosition(x+width, y);

			cellRight.drawOn(this.page);
			
			Line lineRight = new Line();
			lineRight.setColor(isPair ? bgColorPair : bgColorImpair);
			lineRight.setPattern(this.linePattern);
			lineRight.setStartPoint(
					this.offsetLeft + this.leftScaleWidth + width, y);
			lineRight.setEndPoint(this.offsetLeft + this.pageWidth, y);
			lineRight.drawOn(this.page);
	...
	private void timelineEventsDraw() throws Exception {
		...
		double offset = 1;
		int numeroPage = 0;
		this.page = this.pages.get(numeroPage);
		for (int i = 0; i < events.length; i++) {
			SchedulerEvent ev = events[i];
			String bgColor = ev.getBackgroundColor();
			String type = ev.getType();
			
			if(ev.getWeek() >= (26 * (numeroPage + 1))) {
				numeroPage++;
				this.page = this.pages.get(numeroPage);
			}
			...
			double y = this.offsetTop + this.headerHeight + ev.getY()
					* this.weekDayHeight / 100 + (ev.getWeek() - (26 * numeroPage))
					* this.weekDayHeight;
			double width = ev.getWidth() * this.weekDayWidth / 100;
			double height = timeLineEventHeight - 3;
			...
			}else if("eventRaye".equals(type)) {

				int nombreRayure = 2;
				if(width>40) {
					nombreRayure = Double.valueOf(width / 20).intValue();
				}
				
				boolean colorPastel = false;
				for(int j=0; j<nombreRayure; j++) {
					Box cont = new Box();
					cont.setPosition(x, y);
					cont.setSize(width/nombreRayure, height);
					cont.setFillShape(true);
					if ((bgColor.compareTo("transparent") != 0)
							&& ((this.profile.compareTo("fullcolor") == 0) || (this.profile
									.compareTo("color") == 0))) {
						cont.setColor(colorPastel ? RGBColor.getColorPastel(bgColor) : RGBColor.getColor(bgColor));
					} else {
						cont.setColor(eventColor);
					}
					cont.placeIn(eventCont, j*width/nombreRayure, 0);
					cont.drawOn(this.page);
...
//				Box cont2 = new Box();
//				cont2.setPosition(x, y);
//				cont2.setSize(width/2, height);
//				cont2.setFillShape(true);
//				if ((bgColor.compareTo("transparent") != 0)
//						&& ((this.profile.compareTo("fullcolor") == 0) || (this.profile
//								.compareTo("color") == 0))) {
//					cont2.setColor(RGBColor.getColorPastel(bgColor));
//				} else {
//					cont2.setColor(eventColor);
//				}
//				cont2.placeIn(eventCont, width/2, 0);
//				cont2.drawOn(this.page);
			}
...
//			txt.drawOn(this.page);
	...
	private void todayLabelDraw(Page p1) throws Exception {
		this.f1.setSize(12);
		double[] textColor = RGBColor.getColor(this.textColorTodayLabel);
		String today = this.parser.getTodatLabel();
		TextLine todayText = new TextLine(this.f1, today);
		double today_x = this.offsetLeft + 300.0;
