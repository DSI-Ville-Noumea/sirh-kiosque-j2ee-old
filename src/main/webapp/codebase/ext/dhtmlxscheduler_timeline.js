...
scheduler.render_timeline_event = function(ev, attach){
	...
	var cs = (ev.className?ev.className:"");
...
function y_scale(d) {
	...
	for (var i=0; i<this.y_unit.length; i++){
		...
		scheduler._merge(stats, {
			//section 1
			tr_className: this.y_unit[i].classNameTr,
		...
		// section 1
		html+="<tr class='"+stats.tr_className+"' style='"+stats.style_height+"'><td class='"+stats.td_className+"' style='"+stats.style_width+" height:"+(stats.height-1)+"px;'><span>"+stats.td_content+"</span></td>";
